...
from .color_utils.colors import *
from .color_utils.scatter import *
...
def _read_graph(adata: AnnData, graph_type: Optional[str]):
    ...
    if graph_type == "PTS_graph":
        ...
        G = adata.uns[graph_type]["graph"][0]

        print(G)
        graph = list(G.subgraph(c) for c in nx.connected_components(G))[0]
        print(graph,1)
        
        #graph = nx.from_scipy_sparse_array(
        #    adata.uns[graph_type]["graph"][0], create_using=nx.DiGraph
        #)
    else:
        G = adata.uns[graph_type]["graph"][0]
        graph = list(G.subgraph(c) for c in nx.connected_components(G))[0]
        print(graph,2)   
        #graph = nx.from_scipy_sparse_array(adata.uns[graph_type]["graph"][0])
