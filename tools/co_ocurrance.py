...
def co_ocurrance(
    ...
    for c in tqdm(clusters, position=0, leave=True):
        ...
        result, result_boots_mean, result_boots_5, result_boots_95, results_boots_std = export_counts(idx_intervals, clusters, dfkey, n_perms, interval)

        result = decumulate(result).astype('float64')
        results_boots_mean = decumulate(result_boots_mean).astype('float64')
        results_boots_5 = decumulate(result_boots_5).astype('float64')
        results_boots_95 = decumulate(result_boots_95).astype('float64')
        results_boots_std = decumulate(results_boots_std).astype('float64')
...
def export_counts(idx_intervals, clusters, dfkey, n_perms, interval):
    results_dists = []
    results_boots_mean = []
    results_boots_5 = []
    results_boots_95 = []
